...
class Kernels:
        ...
        self.program = cl.Program(ctx, """
            __kernel void fused_kernel(
                ...
                if (gid == 0) {
                    ...
                    float total_last_prices = 0;
                    for (int i = 0; i < num_goods; i++) {
                        total_prices += new_prices[i];
                        total_last_prices += last_prices[i];
                    }
                    float average_price = total_prices / num_goods;
                    float average_last_price = total_last_prices / num_goods;
                    *inflation = (average_price / average_last_price) - 1;

                    // Adjust interest rate based on inflation
                    if (*inflation > 0) {
                        *interest_rate_adjustment = *inflation * 0.01;
                    } else {
                        *interest_rate_adjustment = *inflation * 0.005; // Less aggressive adjustment for deflation
                    }
                    *interest_rate += *interest_rate_adjustment;

                    // Calculate new prices based on money supply and interest rate
                    float new_money_supply = (*interest_rate < 0.05) ? *money_supply * (1 + (1 / (*interest_rate * 2))) : 1 - *interest_rate * 5;
                    for (int i = 0; i < num_goods; i++) {
    ...
    def fused_kernel(self, banks, consumers, goods, interest_rate, buy_amounts, sell_amounts, gdp_growth, unemployment_rate, interest_rate_adjustment, bond_price, money_supply, salary, money_supply_increment, last_prices):
        ...
        last_prices_buf = cl.Buffer(self.ctx, self.mf.READ_WRITE | self.mf.COPY_HOST_PTR, hostbuf=last_prices)
        ...
        self.program.fused_kernel(self.queue, (num_consumers + num_banks,), None,
                                  ...
                                  new_prices_buf, last_prices_buf, inflation_buf, gdp_growth_buf, unemployment_rate_buf, interest_rate_adjustment_buf,
                                  bond_price_buf, money_supply_buf, salary_buf, bank_bond_buying_buf,
                                  bank_bond_selling_buf, updated_money_supply_buf, updated_salary_buf,
        ...
        return banks, consumers, goods, interest_rate, inflation[0], bank_bond_buying[0], bank_bond_selling[0], updated_money_supply[0], updated_salary[0]